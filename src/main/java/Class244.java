...
public class Class244 {
	...
	public byte[][][] terrainSettings;

	public void method2319(int i) {
		try {
			for (int i_0_ = 0; i_0_ < terrainSettings.length; i_0_++) {
				for (int i_1_ = 0; i_1_ < terrainSettings[0].length; i_1_++) {
					for (int i_2_ = 0; i_2_ < terrainSettings[0][0].length; i_2_++)
						terrainSettings[i_0_][i_1_][i_2_] = (byte) 0;
				}
			}
	...
	public boolean method2320(int i, int i_3_, int i_4_) {
		try {
			if (i < 0 || i_3_ < 0 || i >= terrainSettings[1].length || i_3_ >= terrainSettings[1][i].length)
				return false;
			if ((terrainSettings[1][i][i_3_] & 0x2) != 0)
				return true;
			return false;
		} catch (RuntimeException runtimeexception) {
	...
	public boolean method2321(int i, int i_5_, int i_6_, int i_7_, byte i_8_) {
		try {
			if (0 != (terrainSettings[0][i_6_][i_7_] & 0x2))
				return true;
			if (0 != (terrainSettings[i_5_][i_6_][i_7_] & 0x10))
				return false;
	...
	public Class244(int i, int i_9_, int i_10_) {
		terrainSettings = new byte[i][i_9_][i_10_];
	}

	int method2322(int i, int i_11_, int i_12_, int i_13_) {
		try {
			if ((terrainSettings[i][i_11_][i_12_] & 0x8) != 0)
				return 0;
			if (i > 0 && (terrainSettings[1][i_11_][i_12_] & 0x2) != 0)
				return i - 1;
			return i;
		} catch (RuntimeException runtimeexception) {
	...
	static final void method2323(WidgetConfig class105, Class119 class119, ClientScript2 class403, int i) {
		try {
	...
	static final void method2324(ClientScript2 class403, int i) {
		try {
			...
			Interface class298_sub51 = ((Interface) client.aClass437_8841.method5812((long) i_14_));
	...
	public static void method2326(int i) {
		try {
			Atmosphere.method4714(1590487126);
		} catch (RuntimeException runtimeexception) {
			throw Class346.method4175(runtimeexception, new StringBuilder().append("kf.f(").append(')').toString());
		}
	}
		...
		try {
			...
			for (;;) {
				...
				if (1 == i_16_) {
					...
					i_17_ = 1821;//731; //song 742 , archive 731
					//i_17_ = 547; //song 153, archive 547 santa snow
					if (Class300.LOGIN_MUSIC * 782166935 == -1)
						Class300.LOGIN_MUSIC = i_17_ * 154813479;
				}
			}
		} catch (RuntimeException runtimeexception) {
