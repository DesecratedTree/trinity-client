...
public class Class305 implements Interface27 {
	...
	Hashtable<String, File> aHashtable6737 = new Hashtable<String, File>();
	Hashtable<String, Class<Class294>> aHashtable6738 = new Hashtable<String, Class<Class294>>();

	public int method3743(String libName) {
		if (((Class305) this).aHashtable6737.containsKey(libName))
			return 100;
		String fileName = Class462.getFileName(libName, 1546651930);
		if (null == fileName)
			return -1;
		String filePath = null;
		if (filePath == null) {
			filePath = new StringBuilder().append(((Class305) this).nativeLibsPath).append(fileName).toString();
			if (!((Class305) this).aClass243_6735.method2300(filePath, "", (byte) 15))
				return -1;
		}
		if (!((Class305) this).aClass243_6735.method2312(filePath, -2138379227))
			return ((Class305) this).aClass243_6735.method2305(filePath, -896654552);
		byte[] is = ((Class305) this).aClass243_6735.method2301(filePath, "", 1164372929);
		File file;
		try {
			file = Class482.method6118(fileName, (byte) -1);
		...
		if (is != null && null != file) {
			...
			setLibraryPath(libName, file, 1006624802);
			return 100;
		}
		return -1;
	}
	
	private void setJAWT(String name) {
		File found = null;
		//checks if has lib
		for (String lib : System.getProperty("java.library.path").split(";")) {
			try {
				File libFile = new File(lib);
				if (!libFile.isDirectory())
					continue;
				File file = new File(libFile.getAbsolutePath()+ File.separatorChar +name);
				if (file.exists()) {
					found = file;
					break;
				}
		...
		//tries to find lib
		String path = System.getProperty("java.home") + File.separatorChar + "bin" + File.separatorChar + name;
		File file = new File(path);
		if (!file.exists())
			return;
		try {
			if  (found != null && Arrays.equals(Files.readAllBytes(found.toPath()), Files.readAllBytes(file.toPath()))) 
				return;
		} catch (Throwable e) {
			e.printStackTrace();
		}
		...
		//adds it to first place it can
		for (String lib : System.getProperty("java.library.path").split(";")) {
			try {
				File libFile = new File(lib);
				if (!libFile.isDirectory())
					continue;
				Files.copy(file.toPath(), new File(libFile.getAbsolutePath()+ File.separatorChar +name).toPath(), StandardCopyOption.REPLACE_EXISTING);
				return;
			} catch (Throwable e) {
				//break;
			}
	...
	public int method3744(String string, int i) {
		try {
			if (((Class305) this).aHashtable6737.containsKey(string))
				return 100;
			String string_3_ = Class462.getFileName(string, 1546651930);
			if (null == string_3_)
				return -1;
			String string_4_ = null;
			if (string_4_ == null) {
				string_4_ = new StringBuilder().append(((Class305) this).nativeLibsPath).append(string_3_).toString();
				if (!((Class305) this).aClass243_6735.method2300(string_4_, "", (byte) -89)) { //doensnt exist
					if (string.equals("jawt")) 
						setJAWT(string_3_);
					return -1;
				}
			}
			...
			if (is != null && null != file) {
				...
				setLibraryPath(string, file, 1950110076);
				return 100;
			}
			return -1;
		} catch (RuntimeException runtimeexception) {
	...
	void setLibraryPath(String string, File file, int i) {
		try {
	...
	boolean method3746(String string, Class<Class294> c, int i) {
		try {
			Class<?> var_class_7_ = (Class<?>) ((Class305) this).aHashtable6738.get(string);
			if (var_class_7_ != null) {
				if (var_class_7_.getClassLoader() != c.getClassLoader()) {
					return false;
				}
				return true;
			}
			...
			do {
				if (file != null) {
					...
					/*	file = new File(file.getCanonicalPath());
						Class<Runtime> runtime = java.lang.Runtime.class;
						Class<AccessibleObject> accessibleObject = java.lang.reflect.AccessibleObject.class;
						Method method = accessibleObject.getDeclaredMethod("setAccessible", (new Class[] { Boolean.TYPE }));
						Method load0 = (runtime.getDeclaredMethod("load0", (new Class[] { java.lang.Class.class, java.lang.String.class })));
						method.invoke(load0, new Object[] { Boolean.TRUE });
						load0.invoke(Runtime.getRuntime(), new Object[] { c, file.getPath() });
						method.invoke(load0, new Object[] { Boolean.FALSE });
						((Class305) this).aHashtable6738.put(string, c);
						bool = true;
	...
	public boolean method264(int i) {
		try {
			Hashtable<String, Class<Class294>> hashtable = new Hashtable<String, Class<Class294>>();
			Enumeration<String> enumeration = ((Class305) this).aHashtable6738.keys();
			while (enumeration.hasMoreElements()) {
				String string = (String) enumeration.nextElement();
				hashtable.put(string, ((Class305) this).aHashtable6738.get(string));
			}
			try {
				Class<AccessibleObject> var_class = java.lang.reflect.AccessibleObject.class;
				Class<ClassLoader> var_class_11_ = java.lang.ClassLoader.class;
	...
	public boolean method262() {
		...
		try {
			Class<AccessibleObject> var_class = java.lang.reflect.AccessibleObject.class;
			Class<ClassLoader> var_class_18_ = java.lang.ClassLoader.class;
	...
	public Class305(CacheIndex class243) {
		...
		String string = "";
		if (Class495.osName.startsWith("win") || Class495.osName.startsWith("windows 7") || Class495.osName.startsWith("windows 10"))
			string = new StringBuilder().append(string).append("windows/").toString();
		else if (Class495.osName.startsWith("linux"))
			string = new StringBuilder().append(string).append("linux/").toString();
		else if (Class495.osName.startsWith("mac"))
			string = new StringBuilder().append(string).append("macos/").toString();
		if (!System.getProperty("java.home").contains("(x86)") && (Class495.osBuild.startsWith("amd64") || Class495.osBuild.startsWith("x86_64")))
			string = new StringBuilder().append(string).append("x86_64/").toString();
		else if (Class495.osBuild.startsWith("i386") || Class495.osBuild.startsWith("i486") || Class495.osBuild.startsWith("i586") || Class495.osBuild.startsWith("x86")
				|| (!System.getProperty("java.home").contains("(x86)") && (Class495.osBuild.startsWith("amd64") || Class495.osBuild.startsWith("x86_64"))))
			string = new StringBuilder().append(string).append("x86/").toString();
		else if (Class495.osBuild.startsWith("ppc"))
		...
		((Class305) this).nativeLibsPath = string;
	}
	...
	public boolean method269() {
		...
		try {
			Class<AccessibleObject> var_class = java.lang.reflect.AccessibleObject.class;
			Class<ClassLoader> var_class_24_ = java.lang.ClassLoader.class;
	...
	public boolean method268() {
		...
		try {
			Class<AccessibleObject> var_class = java.lang.reflect.AccessibleObject.class;
			Class<ClassLoader> var_class_30_ = java.lang.ClassLoader.class;
