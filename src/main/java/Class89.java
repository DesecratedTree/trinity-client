...
public class Class89 {
	...
	public static Class89 getSprite(CacheIndex class243, int i) { //textures msotly
	...
	public static Class89[] method981(CacheIndex class243, int i) { //barely used
	...
	public static Class89[] method982(byte[] is) {
		...
		if (i == 65535) {
			...
			i = class298_sub53.readUnsignedShort();
		}

		Class89[] class89s = new Class89[i];
		for (int i_9_ = 0; i_9_ < i; i_9_++)
			class89s[i_9_] = new Class89();
		class298_sub53.index = (is.length - (newFormat ? 12 : 7) - i * 8) * 116413311;
		int i_10_ = class298_sub53.readUnsignedShort();
		int i_11_ = class298_sub53.readUnsignedShort();
		int i_12_ = (newFormat ? class298_sub53.readInt((byte)0) : (class298_sub53.readUnsignedByte() & 0xff)) + 1;
		...
		class298_sub53.index = (is.length - (newFormat ? 12 : 7) - i * 8 - (i_12_ - 1) * 3) * 116413311;
			...
			if ((i_23_ & 0x2) == 0) {
				if ((i_23_ & 0x1) == 0) {
					for (int i_24_ = 0; i_24_ < i_22_; i_24_++)
						class89.aByteArray818[i_24_] = class298_sub53.readByte();
				} else {
					for (int i_25_ = 0; i_25_ < class89.anInt812; i_25_++) {
						for (int i_26_ = 0; i_26_ < class89.anInt816; i_26_++)
							class89.aByteArray818[(i_25_ + i_26_ * class89.anInt812)] = class298_sub53.readByte();
					}
				}
			} else {
				boolean bool = false;
				...
				if ((i_23_ & 0x1) == 0) {
					for (int i_27_ = 0; i_27_ < i_22_; i_27_++)
						class89.aByteArray818[i_27_] = class298_sub53.readByte();
					for (int i_28_ = 0; i_28_ < i_22_; i_28_++) {
						byte i_29_ = (class89.aByteArray819[i_28_] = class298_sub53.readByte());
						bool = bool | i_29_ != -1;
					}
				} else {
					for (int i_30_ = 0; i_30_ < class89.anInt812; i_30_++) {
						for (int i_31_ = 0; i_31_ < class89.anInt816; i_31_++)
							class89.aByteArray818[(i_30_ + i_31_ * class89.anInt812)] = class298_sub53.readByte();
					}
					for (int i_32_ = 0; i_32_ < class89.anInt812; i_32_++) {
						for (int i_33_ = 0; i_33_ < class89.anInt816; i_33_++) {
							byte i_34_ = (class89.aByteArray819[i_32_ + i_33_ * class89.anInt812] = class298_sub53.readByte());
							bool = bool | i_34_ != -1;
						}
					}
				}
	...
	private static int[] OSRS_GAMEFRAME = new int[] {12, 169, 784, 
			1033, 1034, 1035,
			1129, 1130, 1206, 1835, 1836, 1837, 
			1126, 1221, 
			...
			,1246, 1247,
			//chatbox part 2
			8564, 8569, 8605,8606, 8607, 8602, 8603, 8604, 8605,
			8597 , 8599 ,8601
			, 8596 , 8598,8600,1136,
	...
	private static int[] OSRS_GAMEFRAME_DISABLE = new int[] {1840, 1841,
			8550, 8551, 8552, 
			//chatbox
			8587, 8588,
			8563, 8565, 8566, 8567, 8568, 8570, 8571, 8572, 8573, 8574, 8575, 8576, 8577,
			8578, 8579, 8580, 8581, 8582, 8583, 8584, 1131
	};

	
	private static boolean isOsrsGF(int i) {
		for (int id : OSRS_GAMEFRAME)
			if (id == i)
				return true;
		return false;
	}
	
	private static boolean isOsrsGfDisable(int i) {
		for (int id : OSRS_GAMEFRAME_DISABLE)
			if (id == i)
				return true;
		return false;
	}
	...
	public static Class89 getSprite(CacheIndex class243, int spriteID, int i_81_) {
		/*if (spriteID == 10247) //squeal to tp
			spriteID = 323;*/
		if (Settings.ENABLE_OSRS_GAMEFRAME && spriteID == 1820) {
			spriteID = 3247;
		}

		if (Settings.ENABLE_OSRS_GAMEFRAME && spriteID >= 156 && spriteID <= 166) 
			spriteID += 20000;
		
		if (Settings.ENABLE_OSRS_GAMEFRAME && spriteID >= 197 && spriteID <= 221) 
			spriteID += 20000;
		
		if (Settings.ENABLE_OSRS_GAMEFRAME && spriteID >= 170 && spriteID <= 176) 
			spriteID += 20000;
		
		if (Settings.ENABLE_OSRS_GAMEFRAME && spriteID == 9167)
			spriteID = 20170;
		
	/*	if (Settings.ENABLE_OSRS_GAMEFRAME && (spriteID == 9191 || spriteID == 9193)) 
			spriteID = 20293;
		
		if (Settings.ENABLE_OSRS_GAMEFRAME && (spriteID >= 652 && spriteID <= 658)) 
			spriteID += 20000;
		
		if (Settings.ENABLE_OSRS_GAMEFRAME && (spriteID == 9192)) 
			spriteID = 20653;
		
		if (Settings.ENABLE_OSRS_GAMEFRAME && (spriteID == 9194)) 
			spriteID = 20654;*/
		
		if (Settings.ENABLE_OSRS_GAMEFRAME && i_81_ == 0)  {
			if (spriteID == -1 || isOsrsGfDisable(spriteID))
				return null;
			if (isOsrsGF(spriteID))
			 i_81_ = 1;
		}
	...
	//	System.out.println(spriteID);
		...
		byte[] is = class243.getFileFromArchive(spriteID, i_81_, (byte) -43);

		if (is == null)
			return null;
		Class89 sprite = method982(is)[0];
		return sprite;
	}
	...
	public static Class89[] getSprites(CacheIndex class243, int i, int i_128_) {
		byte[] is = class243.getFileFromArchive(i, i_128_, (byte) 4);
		if (is == null)
			return null;
		Class89[] sprites = method982(is);
		return sprites;
	}
