	...
	static final void decodeNPCsMasks(short i) {
		try {
			...
			for (int i_0_ = 0; i_0_ < client.anInt8808 * -976358333; i_0_++) {
				...
				NPC npc = ((NPC) (((ObjectValue) client.aClass437_8696.method5812((long) npcIndex)).value));
				...
				if (0 != (mask & 0x4)) { // Hit ? ordinal 7
					npc.hpBarSpriteID = stream.readBigSmart(0);
					...
					if (i_10_ > 0) {
						for (int i_11_ = 0; i_11_ < i_10_; i_11_++) {
							...
							if (i_15_ == 32767) {
								...
								damage = stream.readUnsignedSmart(1723054621);
								i_12_ = stream.readUnsignedSmart(1723054621);
								soakingDamage = stream.readUnsignedSmart(1723054621);
							} else if (32766 != i_15_) {
								damage = stream.readUnsignedSmart(1723054621);
							} else {
								i_15_ = -1;
							...
							npc.method4420(i_15_, damage, i_12_, soakingDamage, client.anInt8884 * 443738891, i_16_, i_17_, (byte) 6);
						}
					}
				}
				if ((mask & 0x1000) != 0) {// ordinal 8
					int i_18_ = (npc.config.models).length;
					int i_19_ = 0;
					if (null != (npc.config.modifiedColors))
						i_19_ = (npc.config.modifiedColors).length;
					int i_20_ = 0;
					if ((npc.config.modifiedTextures) != null)
						i_20_ = (npc.config.modifiedTextures).length;
				...
				if ((mask & 0x4000) != 0) {// ordinal 12
					...
					ClientScriptEnum.method3428(npc, is, is_43_, is_44_, 1644064563);
				}
				...
				if ((mask & 0x800000) != 0) { // Rename - ordinal 18
					...
					if ("".equals(npc.aString10186) || (npc.aString10186.equals(npc.config.name)))
						npc.aString10186 = (npc.config.name);
				}
				...
				if ((mask & 0x200000) != 0) {// ordinal 20
					int i_66_ = (npc.config.headModels).length;
					int i_67_ = 0;
					if (null != (npc.config.modifiedColors)) {
						i_67_ = (npc.config.modifiedColors).length;
					}
					int i_68_ = 0;
					if ((npc.config.modifiedTextures) != null) {
						i_67_ = (npc.config.modifiedTextures).length;
					}
				...
				if ((mask & 0x40000) != 0) {// Change combat level - ordinal 21
					npc.combatLevel = (stream.readUnsignedShortLE((byte) 16) * 933118661);
					if (npc.combatLevel * 1817570317 == 65535) {
						npc.combatLevel = (npc.config.combatLevel) * 1520279523;
					}
				}
				if ((mask & 0x1) != 0) {// Transform NPC id - ordinal 22
					if (npc.config.method6242((byte) 32)) {
						Class244.method2330(npc, (byte) 7);
					}
					npc.setConfig((Class15.aClass507_224.method6269(stream.readBigSmart(1235052657), 384117949)), 1598792788);
					npc.method4421((npc.config.anInt6187) * -2095128707, -2141370583);
					npc.anInt10115 = -1186616623 * (((npc.config.opcode103V) * -1927065533) << 3);
					if (npc.config.method6242((byte) 23)) {
						AnimationConfig.method4884(npc.plane, npc.scenePositionXQueue[0], npc.scenePositionYQueue[0], 0, null, npc, null, (byte) 68);
					}
				}
			}
		} catch (RuntimeException runtimeexception) {
