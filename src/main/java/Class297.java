...
public class Class297 {
	...
	static final void decodePlayerMasks(RsBitsBuffer stream, int playerIndex, Player player, int mask) {
		try {
			...
			if (0 != (mask & 0x8)) { // hits
				player.hpBarSpriteID = stream.readBigSmart(0);
			...
			if ((mask & 0x20000) != 0) {
				...
				if ((i_40_ & 0x1) != 0)
					GrabFolder.method2282(2, i_40_, player.method4470(true, -2017419584), player.method4475(false, (byte) -87), player.aString10200, string, 252423564);
			...
			if (0 != (mask & 0x4000)) {
				...
				ClientScriptEnum.method3428(player, is, is_43_, is_44_, 1644064563);
			}
			...
			if (0 != (mask & 0x100)) { // forcetalk
				...
				if (player == Class287.myPlayer)
					GrabFolder.method2282(2, 0, player.method4470(true, -1880046656), player.method4475(false, (byte) -103), player.aString10200, string, -588173673);
