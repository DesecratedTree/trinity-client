...
public class Class462 {
	static CachingHashMap aClass348_5682 = new CachingHashMap(32);
	...
	static String getFileName(String string, int i) {
		try {
			if (Class495.osName.startsWith("win"))
				return new StringBuilder().append(string).append(".dll").toString();
			if (Class495.osName.startsWith("linux"))
				return new StringBuilder().append("lib").append(string).append(".so").toString();
			if (Class495.osName.startsWith("mac"))
	...
	static final void method5991(ClientScript2 class403, int i) {
		try {
			...
			WidgetConfig class105 = Class50.getIComponentDefinitions(i_0_, (byte) -48);
