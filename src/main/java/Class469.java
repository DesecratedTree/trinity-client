...
public class Class469 {
	...
	static void method6047(GraphicsToolkit class_ra, Interface_ma interface_ma, int i, int i_5_, int i_6_, int i_7_, int i_8_) {
		try {
			...
			if (Class301_Sub1.anInt7624 * -1289394455 < 100) {
				...
				WorldMap.method3709(i_12_ + WorldMap.baseX, i_13_ + WorldMap.baseY, WorldMap.baseX + i_14_, i_15_ + WorldMap.baseY, i, i_5_, i_6_ + i, 1 + (i_5_ + i_7_));
	...
	static WorldMapArea getWorldMapArea(CacheIndex index, int archive, int file, int i_21_) {
		try {
			RsByteBuffer buffer = new RsByteBuffer(index.getFileFromArchive(archive, file, (byte) 3));
			WorldMapArea area = (new WorldMapArea(file, buffer.readString(-1014876746), buffer.readString(-346433981), buffer.readInt((byte) -81), buffer.readInt((byte) 27), buffer.readUnsignedByte() == 1, buffer.readUnsignedByte(), buffer.readUnsignedByte()));
			int hotlinkCount = buffer.readUnsignedByte();
			for (int i_23_ = 0; i_23_ < hotlinkCount; i_23_++) {
				area.areaBounds.add(new WorldMapAreaBound(buffer.readUnsignedByte(), buffer.readUnsignedShort(), buffer.readUnsignedShort(), buffer.readUnsignedShort(), buffer.readUnsignedShort(), buffer.readUnsignedShort(), buffer.readUnsignedShort(), buffer.readUnsignedShort(), buffer.readUnsignedShort()), 2080542038);
			}
			
			if (area.fileName.equals("main")) {
				//zeah
				area.areaBounds.add(new WorldMapAreaBound(0, 1099, 3287, 1947, 4080, 1099, 3287, 1947, 4080), 2080542038);
			}
			
			area.method3454(955144381);
			return area;
		} catch (RuntimeException runtimeexception) {
