...
public class Class505 {
	...
	public static int method6265(int i) {
		try {
			boolean trySoftware = false; //true
			boolean tryOpenGL = true;
			boolean tryDX = true;
			boolean tryModernOpenGL = false;
			...
			if (Class11.triedSoftware)
				trySoftware = false;
			if (Class11.triedLegacyOpenGL)
				tryOpenGL = false;
			if (Class11.triedDX)
				tryDX = false;
			if (Class11.triedModernOpenGL)
				tryDX = false;
			if (!trySoftware && !tryOpenGL && !tryDX && !tryModernOpenGL)
				return Class407.method4964(571472892);
			int softwareModeBenchmark = -1;
			int legacyGLBenchmark = -1;
			int dxBenchmark = -1;
			int modernGLBenchmark = -1;
			
			
			if (tryDX) { //dx
				try {
					dxBenchmark = ItemConfig.method6043(3, 1000, -2071277019);
					if (Class422_Sub25.preferences.graphicsMode.method5677(-491668376) == 3) {
						...
						case 4318:
							tryOpenGL = tryOpenGL & l >= 64425238954L;
							break;
						case 4098:
							tryOpenGL = tryOpenGL & l >= 60129613779L;
							break;
						}
					}
					if (dxBenchmark != -1)
						return Class128_Sub1.method1439(3, dxBenchmark, -683313294);
				} catch (Exception exception) {
					/* empty */
				}
			}
			...
			if (tryModernOpenGL) {
				try {
					modernGLBenchmark = ItemConfig.method6043(5, 1000, -1485617713);
				} catch (Exception exception) {
					/* empty */
					Settings.ENABLE_MODERN_OPENGL = false;
					tryOpenGL = true;
				}
			}
			
			if (modernGLBenchmark == -1 && tryOpenGL) {
				try {
					legacyGLBenchmark = ItemConfig.method6043(1, 1000, -1485617713);
					if (legacyGLBenchmark != -1) //set always opengl as default since mem leak fixed and less graphic bugs,no need to benchmark everything.
						return Class128_Sub1.method1439(1, legacyGLBenchmark, -1159261404);
				} catch (Exception exception) {
					/* empty */
				}
			}
			...
			if (trySoftware) { //software mode
				try {
					softwareModeBenchmark = ItemConfig.method6043(2, 1000, 280501708);
				} catch (Exception exception) {
					/* empty */
				}
			}
			if (softwareModeBenchmark == -1 && -1 == legacyGLBenchmark && -1 == dxBenchmark && modernGLBenchmark == -1)
				return Class407.method4964(571472892); //safe mode
			
			dxBenchmark *= 1.1F;
			legacyGLBenchmark *= 1.1F;
			modernGLBenchmark *= 1.1F;
			
			if (softwareModeBenchmark != -1 && softwareModeBenchmark > dxBenchmark && softwareModeBenchmark > legacyGLBenchmark  && softwareModeBenchmark > modernGLBenchmark)
				return Class353.method4252(softwareModeBenchmark, -2143732206);
			if (dxBenchmark > legacyGLBenchmark && dxBenchmark > modernGLBenchmark)
				return Class128_Sub1.method1439(3, dxBenchmark, -683313294);
			if (modernGLBenchmark > legacyGLBenchmark) 
				return Class128_Sub1.method1439(5, modernGLBenchmark, -1159261404);
			return Class128_Sub1.method1439(1, legacyGLBenchmark, -1159261404);
		} catch (RuntimeException runtimeexception) {
	...
	public static void method6267(Class298 class298, Class298 class298_85_, byte i) {
		try {
			if (class298.first != null)
				class298.remove(-1460969981);
			class298.first = class298_85_.first;
			class298.last = class298_85_;
