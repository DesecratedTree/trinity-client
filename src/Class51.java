/* Class51 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */

public class Class51 {
	public String aString483;
	public String aString484;
	public static int anInt485 = 4098;
	public static int anInt486 = 32902;
	public static int anInt487 = -1;
	public int anInt488;
	public static int anInt489 = 4318;
	public int anInt490;
	public static int anInt491 = 0;
	public long aLong492;

	public Class51(int i, String string, int i_0_, String string_1_, long l) {
		anInt488 = i * -55586639;
		aString483 = string;
		anInt490 = i_0_ * 1288833183;
		aString484 = string_1_;
		aLong492 = l * -1503932947191017445L;
	}

	public static int method633(Class330_Sub36_Sub10 class330_sub36_sub10, int i) {
		try {
			if (Class484.aBoolean5782)
				return 6;
			if (class330_sub36_sub10 == null)
				return 0;
			int i_2_ = (400571611 * ((Class330_Sub36_Sub10) class330_sub36_sub10).anInt9699);
			if (Class228.method2313(i_2_, (byte) 37))
				return 1;
			if (Class487_Sub1.method6041(i_2_, 536870912))
				return 2;
			if (Class391.method4607(i_2_, (byte) 117))
				return 3;
			if (Class531.method6298(i_2_, 1481449874))
				return 4;
			if (Class119.method1437(i_2_, -2041984243))
				return 7;
			if (16 == i_2_)
				return 8;
			return 5;
		} catch (RuntimeException runtimeexception) {
			throw Class476.method5964(runtimeexception, new StringBuilder().append("ca.aq(").append(')').toString());
		}
	}

	public static void method634(int i) {
		try {
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub20_7888), 1, 2701663);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub20_7889), 1, -897573921);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub6_7882), 1, -1072082305);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub27_7881), 1, -1814357451);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub4_7883), 0, -697445989);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub11_7879), 0, 1334843040);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub22_7892), 0, -1104165862);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub28_7885), 0, -1249793062);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub13_7893), 0, -747517208);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub2_7868), 0, 782433921);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub25_7896), 0, 1121071485);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub3_7880), 0, 205116080);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub9_7899), 0, -1636997640);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub9_7874), 0, 1577216649);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub12_7906), 0, -817503708);
			Class448.aClass330_Sub50_5555.method3856(Class448.aClass330_Sub50_5555.aClass464_Sub18_7877, Class134.aClass134_1515.anInt1516 * -1961860223, -1427587640);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub21_7875), 0, 728208785);
			if (null != Class361.aClass_ra3793 && Class361.aClass_ra3793.v() && Class361.aClass_ra3793.cg())
				Class361.aClass_ra3793.ca();
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub7_7891), 0, 1840486995);
			Class325.method3212(1304623711);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub5_7890), 2, -1117921123);
			Class448.aClass330_Sub50_5555.method3856((Class448.aClass330_Sub50_5555.aClass464_Sub16_7900), 2, 1714896324);
			Class22.method473((byte) -11);
			client.aClass304_9030.method3008(1464687066).method4582(-278080136);
			client.aBoolean8992 = true;
		} catch (RuntimeException runtimeexception) {
			throw Class476.method5964(runtimeexception, new StringBuilder().append("ca.n(").append(')').toString());
		}
	}

	static int method635(String string, byte[] is, int i, int i_3_) {
		try {
			int i_4_ = i;
			int i_5_ = string.length();
			for (int i_6_ = 0; i_6_ < i_5_; i_6_ += 4) {
				int i_7_ = Class470.method5864(string.charAt(i_6_), (byte) -118);
				int i_8_ = (i_6_ + 1 < i_5_ ? Class470.method5864(string.charAt(1 + i_6_), (byte) 62) : -1);
				int i_9_ = (2 + i_6_ < i_5_ ? Class470.method5864(string.charAt(i_6_ + 2), (byte) -23) : -1);
				int i_10_ = (3 + i_6_ < i_5_ ? Class470.method5864(string.charAt(3 + i_6_), (byte) -3) : -1);
				is[i++] = (byte) (i_7_ << 2 | i_8_ >>> 4);
				if (i_9_ == -1)
					break;
				is[i++] = (byte) ((i_8_ & 0xf) << 4 | i_9_ >>> 2);
				if (-1 == i_10_)
					break;
				is[i++] = (byte) ((i_9_ & 0x3) << 6 | i_10_);
			}
			return i - i_4_;
		} catch (RuntimeException runtimeexception) {
			throw Class476.method5964(runtimeexception, new StringBuilder().append("ca.j(").append(')').toString());
		}
	}

	static final void method636(Class430 class430, int i) {
		try {
			Class426 class426 = (((Class430) class430).aBoolean4398 ? ((Class430) class430).aClass426_4384 : ((Class430) class430).aClass426_4370);
			IComponentDefinitions iComponentDefinitions = ((Class426) class426).aClass114_4345;
			((Class430) class430).anIntArray4387[((((Class430) class430).anInt4376 += -1390004513) * 1632830751 - 1)] = (1 == iComponentDefinitions.anInt1238 * -276075115 ? -1576413395 * iComponentDefinitions.anInt1239 : -1);
		} catch (RuntimeException runtimeexception) {
			throw Class476.method5964(runtimeexception, new StringBuilder().append("ca.pr(").append(')').toString());
		}
	}

	static final void method637(Class430 class430, int i) {
		try {
			((Class430) class430).anIntArray4387[((((Class430) class430).anInt4376 += -1390004513) * 1632830751 - 1)] = Class448.aClass330_Sub50_5555.aClass464_Sub27_7881.method5832(2004126510) ? 1 : 0;
		} catch (RuntimeException runtimeexception) {
			throw Class476.method5964(runtimeexception, new StringBuilder().append("ca.aph(").append(')').toString());
		}
	}

	public static void method638(int i, int i_11_) {
		try {
			Class354_Sub1.anInt7987 = 1284625249 * i;
			Class354_Sub1.anInt7980 = 651096279;
			Class448.anInt5556 = -1741404779;
			Class101.anInt6456 = 1758134756;
		} catch (RuntimeException runtimeexception) {
			throw Class476.method5964(runtimeexception, new StringBuilder().append("ca.cf(").append(')').toString());
		}
	}
}
